...
Additional relevant factors for the paper are frequencies of words in adults’ and child directes speech and age of each word's reproducing.
...
The diagrams below demonstrate outliers (categories children reproduce earlier or later than significant majority of the studied words) separated by categories.
...
The second hypothesis is examined with *Pearson's correlation coefficient*. As we can observe above, the data have small amount of outliers and tied values, so it appears to be the most suitable evaluation of connection between word frequencies and age.
...
Other results are complicated. To understand why the test and the graphs demonstrate correlation between adults' speech and age we should perform additional researches. One of possible explanations is that it is more important which words infants and toddlers hear from adults' different talks than from talks directed to them. Other one is that adults do not speak among themselves about topics and ideas which they could use in child directed speech. So, the frequencies of words which adults use in CDS are less in adults' speech than in CDS.
