...
Выбрала Анову, так как она может показать, есть ли взаимосвязь между возрастом и частотностями слов из разных лексических категорий, а также построю линейную регрессию
...
```{r}
TukeyHSD(categories_acquisition)
```
```{r}
...
ggplot(df, aes(x = target_cats$Lex_cat, y = target_cats$AoA)) +
  geom_point(size = 4) 
  #geom_errorbar(aes(ymax = U, ymin = L))

```
library(sciplot)
lineplot.CI(data = target_cats, 
            response = target_cats$AoA, 
            x.factor = target_cats$Lex_cat)
```
```
```{r}
