...
The second hypothesis is examined on *Pearson's correlation coefficient*. As we can observe above, the data have small amount of outliers and tied values, so it appears to be the most suitable evaluation of connection between word frequencies and age.
...
On this graph *verbs-nouns* and *verbs-adjectives* intervals cross the 0 line, and adjusted p-values of that pairs confirm that difference between that parts of speech is not statistical significant. It could be explained by the fact that children might acquire verbs together with nouns and adjectives in collocations. But other parts of speech demonstrate that some categories are reproduced by children of different ages. Here is the visualisation of connection between parts of speech and their acquisition.
...
In the tests below the author investigates if there appears to be correlation between frequency of words in adults' speech and age, frequency of words in child directed speech and age, both of that factors and age.
...
The first result proves that there appears to be correlation, but the coefficient is close to zero value. Though p-value is less than 0.05, it is rather close to 0.05. In the third result we can observe similar situation. The null hypothesis in these results should be rejected, but we do not see strong correlation between this factors. The author also emphasises that the second general hypothesis does not approved with this results. It was suggested that children have higher probability to learn earlier more frequent words but we see contrasting results. 

The second p-value is more that 0.05. It means that we should accept the null hypothesis and deny the second main hypothesis in case of child directed speech frequency.
...
The provided graph represents that there appears to be large spread of frequencies' values. It is a problem for finding the most reliable regression's line. As we are able to notice, p-value for F-statistics is 0.03359 and p-value for influece of frequencies is 0.0336. The graph with straight regression line is demonstrated below.
...
ggplot(data = main_data, aes(x = Freq, y = AoA)) + 
  ...
  labs(x = "frequency in adult speech",
       y = "age of acquisition",
       title = "Frequency in adult speech and age") +
  geom_smooth(method=lm)
```

In the picture below we are able to detect residuals. The most part of the main data is below or above zero line.
...
ggplot(data = main_data, aes(x = CDS_freq, y = AoA)) + 
  ...
  labs(x = "frequency in child directed speech",
       y = "age of acquisition",
       title = "Frequencies in CDS and age") +
  geom_smooth(method=lm)
...
ggplot(data = main_data_model, aes(Freq + CDS_freq + Lex_catclosedclass + Lex_catverbs + Lex_catnouns + Lex_catadjectives, AoA, color=Lex_cat))+
  ...
  labs(x = "frequency in parents' speech",
       y = "age of acquisition",
       title = "Frequencies + POS and age")+ scale_colour_discrete("POS/category") +
  theme(axis.text.x = element_text(size=8, angle=47, margin = margin(t=10)))+
  scale_y_continuous(breaks=seq(0,60,3))+
  scale_x_continuous(trans = log_trans(), breaks=c(1,5, 10, 100, 1e3, 1e4, 1e5, 1e6, 1e7, 2e7))
```
...
ggplot(data = main_data_model, aes(x = Freq + CDS_freq + Lex_catclosedclass + Lex_catverbs + Lex_catnouns + Lex_catadjectives, y = AoA)) + 
  ...
  labs(x = "frequency in parents' speech",
       y = "age of acquisition",
       title = "Frequencies + POS and age") +
  geom_smooth(method=lm)
  
```

```{r}
linreg5.resid = resid(linreg5)

plot(main_data$AoA, linreg5.resid, 
     ylab="Residuals", xlab="age", 
     main="Frequencies + POS and age") 
abline(0, 0) 
```
...
Hansen, P (revised). What makes a word easy to acquire? The effects of word class, frequency, imageability and phonological neighbourhood density on lexical development. Manuscript submitted for publication.
